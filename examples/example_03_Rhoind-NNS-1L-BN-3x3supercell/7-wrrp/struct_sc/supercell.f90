...
!=============================================================================
!subroutine get_sc_grid(a,nfft,rcart,sc_size,rp_ind,sc_a,irp,sc2uc_idx,scvec2real,sc_rcart)
!
!    integer, intent(in) :: nfft(6), sc_size(3), rp_ind(3)
!    real(DP), intent(in) :: a(3,3) 
!    real(DP), intent(in) :: rcart(:,:)
!    real(DP), intent(out) :: sc_a(3,3)
!    integer, intent(out) :: irp, sc2uc_idx(:), scvec2real(:,:)
...
!    enddo
!    
!    !! save irp here based on input...
!    ! TODO
!    irp = (rp_ind(1)-1)*sc_nfft(2)*sc_nfft(3) + (rp_ind(2)-1)*sc_nfft(3) + rp_ind(3)
!
...
!!    print *, "Inteirpolating onto coarse grid..."
!    ! Inteirpolation onto a coarse grid in the sc with x,y,z grid points
...
!!                    call inteirpol(coarse,sc_ngrid,nmax,wfnvint,sc_data)
...
!=============================================================================
subroutine get_sc_fftbox(fftbox,irp,nr_sc,scvec2real,rvec2real,sc2uc_idx,sc_fftbox)
    
    complex(DPC), intent(in) :: fftbox(:,:,:,:,:,:)
    integer, intent (in) :: irp, nr_sc, scvec2real(:,:), rvec2real(:,:), &
                            sc2uc_idx(:)
    complex(DPC), intent(in out) :: sc_fftbox(:,:,:)
    integer :: r

    ! 2019.01.10 Modify such that only need to map to 3d box.
    ! For each data point in sc, map it to the unit cell
!$omp parallel do default(private) shared(sc_fftbox,fftbox,scvec2real,rvec2real,sc2uc_idx,nr_sc,irp)
    do r=1,nr_sc
       !     write(6,*) rvec2real(1,sc2uc_idx(r)),rvec2real(2,sc2uc_idx(r)),rvec2real(3,sc2uc_idx(r))
            ...
            fftbox(rvec2real(1,sc2uc_idx(r)),rvec2real(2,sc2uc_idx(r)),rvec2real(3,sc2uc_idx(r)),&
                rvec2real(1,sc2uc_idx(irp)),rvec2real(2,sc2uc_idx(irp)),rvec2real(3,sc2uc_idx(irp)))
    enddo
!$omp end parallel do
